...
**SEG-001:** Un usuario intenta acceder a recursos restringidos sin estar autenticado.

#### Táctica: Resistir ataques  

- **Autenticar a los actores:** Utilizar Auth0 para gestionar la autenticación y autorización, implementando OAuth 2.0 para asegurar que las aplicaciones y servicios interactúen de forma segura y controlada.
- **Sustento:** Asegurar que solo los usuarios autenticados puedan acceder a recursos restringidos es fundamental para proteger la integridad y la confidencialidad de los datos. Implementar una autenticación robusta y segura ayuda a prevenir accesos no autorizados y protege la plataforma contra posibles ataques.
- **Implementación Técnica:**  Configurar Auth0 para gestionar el proceso de inicio de sesión y autorización en la plataforma educativa. Implementar OAuth 2.0 siguiendo el flujo de autorización adecuado (Authorization Code Flow, Implicit Flow, etc.).

**SEG-002:**  Se detecta un intento de acceso no autorizado a los datos del sistema.

#### Táctica: Detectar ataques

- **Detectar intrusos:** Implementar Splunk para monitorear y analizar registros en tiempo real, permitiendo la detección de comportamientos sospechosos y la respuesta rápida a posibles amenazas.
- **Sustento:** Detectar y responder rápidamente a intentos de acceso no autorizado es crucial para proteger los datos sensibles y mantener la integridad del sistema. Utilizar herramientas de monitoreo en tiempo real como Splunk permite identificar y mitigar amenazas antes de que puedan causar daños significativos.
- **Implementación Técnica:** Configurar Splunk para recolectar y analizar los logs del sistema, incluyendo accesos, errores y eventos de seguridad. Implementar reglas y alertas en Splunk para identificar patrones de comportamiento sospechoso y notificar a los administradores en tiempo real.
